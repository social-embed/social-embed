...
export function App() {
  const [serializedHtml, setSerializedHtml] = useState("<p>Paste a URL or insert a sample embed.</p>");
  ...
  return (
    <main data-testid="app-root" style={{ fontFamily: "sans-serif", margin: "0 auto", maxWidth: 960, padding: 24 }}>
      ...
      <p>
        This example keeps one editor node shape and one serialized output shape:
        {" "}
        <code>{sampleMarkup}</code>
      </p>

      <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
        <button data-testid="insert-embed" onClick={insertSampleEmbed} type="button">
          Insert Sample Embed
        </button>
      </div>
