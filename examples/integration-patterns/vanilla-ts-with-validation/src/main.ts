...
export function bootstrapApp(container: HTMLElement) {
  container.innerHTML = `
    <main data-testid="app-root" style="font-family: sans-serif; margin: 0 auto; max-width: 960px; padding: 24px;">
      <h1>Vanilla TS + Zod Validation Example</h1>
      <p>Validates URLs through three gates before rendering: URL format (Zod), provider recognition, and allow list.</p>
      <p><strong>Allow list:</strong> ${ALLOWED_PROVIDERS.join(", ")}</p>
      ...
      <div style="display: flex; gap: 12px; margin-top: 8px;">
        ...
        <button data-testid="sample-malformed" type="button">Use Malformed URL</button>
      </div>
  ...
  const sampleMalformedButton = container.querySelector(
    '[data-testid="sample-malformed"]',
  ) as HTMLButtonElement;

  function renderStatus() {
    const result = validateAndBuildEmbed(input.value);

    if (result.isValid) {
      status.textContent = `Accepted provider: ${result.providerName}`;
      preview.innerHTML = `<o-embed data-testid="validated-embed" url="${result.embedUrl}"></o-embed>`;
      return;
    }
  ...
  sampleMalformedButton.addEventListener("click", () => {
