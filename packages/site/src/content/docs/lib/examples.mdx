...
The `Embed` class provides a rich wrapper around embed output with convenient methods:

```ts
import { toEmbed } from "@social-embed/lib/browser";

// Create an Embed instance from a URL
const embed = toEmbed("https://youtu.be/dQw4w9WgXcQ");

if (embed) {
  // Get the embed URL
  console.log(embed.toUrl());
  // => "https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ"

  // Get complete HTML for rendering
  ...
  // Get structured data for serialization
...
```ts
const embed = toEmbed("https://open.spotify.com/track/abc123");

if (embed) {
  console.log(embed.provider);  // "Spotify"
  console.log(embed.data);      // { contentType: "track", id: "abc123" }
  console.log(embed.nodes);     // Direct access to output nodes
}
```

---

## 10. Browser Convenience Functions
...
// Register globally (affects all components)
register(TikTokMatcher);

// Optionally set priority (higher = matches first)
...
## 11. RegistryStore (Reactive State)

The `RegistryStore` provides reactive state management for dynamic matcher registration:

```ts
import { RegistryStore, defineIframeMatcher } from "@social-embed/lib";

const store = RegistryStore.withDefaults();

// Subscribe to registry changes
const unsubscribe = store.subscribe((registry) => {
  console.log("Registry updated:", registry.list().map(m => m.name));
});

// Mutations trigger subscribers
store.register(MyCustomMatcher);
// => "Registry updated: [YouTube, Vimeo, ..., MyCustomMatcher]"
...
import { defaultStore, toEmbed } from "@social-embed/lib/browser";

function EmbedComponent({ url }: { url: string }) {
  const [embed, setEmbed] = useState(() => toEmbed(url));

  useEffect(() => {
    // Re-render when matchers change
    return defaultStore.subscribe(() => {
      setEmbed(toEmbed(url));
    });
  }, [url]);

  if (!embed) return <p>Unsupported URL</p>;

  // Render using iframe node data directly
  const node = embed.toNodes()[0];
  if (node.type === "iframe") {
    return <iframe src={node.src} {...node.attributes} />;
  }
  return null;
}
```
...
store.has("YouTube");  // true

// Get current registry (immutable)
const registry = store.current;

// Get matcher count
store.size;  // 7 (built-in matchers)
```
...
- **Reactive**: Use `RegistryStore` for dynamic matcher management
