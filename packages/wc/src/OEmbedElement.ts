...
export class OEmbedElement extends LitElement {
  ...
  static properties = {
    ...
    height: { type: String },
    provider: { attribute: false, type: Object },
    title: { type: String },
    url: { type: String },
    width: { type: String },
  };
   ...
   * Declared in static properties as `attribute: false` — set via the JS API only.
   */
  ...
  public renderYouTube(): TemplateResult {
    ...
    const defaults = this.getDefaultDimensions(this.provider);
